...
def simplify(clauses):
    ...
    # Remove tautologies (for instance, B or (not, B))
    ...
    clauses = [x for x in clauses if x != "delete"]
...
def get_clauses(tree, flag):
    ...
    # Main part - The main idea is that we can only get clauses from subtrees with root or, neg or if they are
    # atoms. Subtrees with root or might have several children. With this in mind, whenever get clauses is called
    # a flag is passed, corresponding to the root node of the caller. If the caller is an and or it is the beginning
    # of the process, then they should print the respective clause. If the caller is an OR, then they should return
    # a value. This is done until every clause has been printed to the system output.
    ...
    if tree.data == "and":
        ...
        # If an OR calls an AND, than the three is built wrongly and there is no need in continuing
        if flag == "or":
            print("Tree of this clause is built incorrectly. There are ANDs inside ORs. The program will exit.")
            exit()
